...
function norm(s) {
  ...
  return {
    ...
    timestamp: s.opened_at || s.timestamp || '',
    closed_at: s.result?.exitAt || s.closed_at || '',
    exit_price: s.result?.exitPrice ?? (s.exit_price ?? 0),
    pnl_percent: s.result?.pnlPercent ?? (s.pnl_pct ?? s.pnl_percent ?? (s.status !== 'open' && s.status !== 'active' ? 0 : undefined)),
